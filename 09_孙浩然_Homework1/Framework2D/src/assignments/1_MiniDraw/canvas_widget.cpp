...
{
    //std::cout << "shape_list_ size: " << shape_list_.size() << std::endl;
...
void Canvas::mouse_click_event()
{
    ...
    //         current_shape_ ->add_control_point(tmp.x,tmp.y);
    //     }
    // }
    ...
    if(shape_type_==USTC_CG::Canvas::kPolygon){
        if(!draw_status_){
            ...
            // if (auto polygon = std::dynamic_pointer_cast<Polygon>(current_shape_))
            // {
            //     ImVec2 tmp = mouse_pos_in_canvas();
            //     polygon->add_control_point(tmp.x,tmp.y);  //现在可以调用,to prevent the function not adding the virtual,无法使用多态
            // }
            ImVec2 tmp = mouse_pos_in_canvas();
            current_shape_ ->add_control_point(tmp.x,tmp.y);
